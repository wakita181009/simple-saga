...
class TestSyncCompensation:
    """Test sync saga compensation behavior."""
    ...
    def test_compensation_on_failure(self):
        """Test compensation runs when step fails."""
        compensation_calls = []
    ...
    def test_compensation_reverse_order(self):
        ...
        assert compensation_log == [3, 2, 1]

    def test_compensation_with_additional_args(self):
        """Test compensation with additional arguments."""
        received_values = []
        ...
        with pytest.raises(ValueError):
            with SyncSaga() as saga:
                saga.step(
                    ...
                    compensation_args=("extra_data",),
                )
                saga.step(action=failing_action, compensation=lambda: None)
